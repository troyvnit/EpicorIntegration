...
/****** Object:  StoredProcedure [dbo].[sptyx_DMSCustInfo]    Script Date: 8/19/2017 10:27:49 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sptyx_DMSCustInfo]              
...
SELECT C.Company
		...
		, Bal.OverDue
		, Prc.ListCode
		, (CASE WHEN C.SysRevID IS NOT NULL THEN UPPER(sys.fn_sqlvarbasetostr(CONVERT(VARBINARY,C.SysRevID))) ELSE '' END) as CusSysRevID
		, (CASE WHEN Prc.SysRevID IS NOT NULL THEN UPPER(sys.fn_sqlvarbasetostr(CONVERT(VARBINARY,Prc.SysRevID))) ELSE '' END) as PrcSysRevID		
		, '' Option1
		, '' Option2
		, '' Option3
		, '' Option4
		, '' Option5
FROM erp.Customer C
LEFT JOIN #ARBalance Bal ON C.Company = Bal.Company AND C.CustNum =  Bal.Custnum
LEFT JOIN erp.CustomerDiscPriceLst Prc ON C.Company = Prc.Company AND C.CustNum = Prc.CustNum
